...
FViewer::FViewer(const TextureOptions& opts)
{
    ...
    { // load background texture
        cv::Mat img = cv::imread(texNames[0],CV_LOAD_IMAGE_UNCHANGED);
        assert( !img.empty() );
    ...
    if( texNames.size() > 1 )
    { // load rigid texture
        cv::Mat img = cv::imread(texNames[1],CV_LOAD_IMAGE_UNCHANGED);
        assert( !img.empty() );
...
void FViewer::draw(void)
{
    ...
    if( (0 == tex_draw_opts.fire_disp_mode.compare("frozen") ) &&
            ( 0 == tex_draw_opts.rigid_disp_mode.compare("off") ) )
    {
        glPushMatrix();
        {
        ...
        glPopMatrix();
    } else if( 0 == tex_draw_opts.fire_disp_mode.compare("on") )
    {
        glPushMatrix();
        {
            glRotated(45.0,0,1,0);
